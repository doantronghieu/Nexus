...
# Function to process imports.py files
process_imports_file() {
    local file="$1"
...
# Find all imports.py files in the apps directory and its subdirectories
find "$ROOT_DIR/apps" -name "packages.py" -type f | while read -r file; do
    process_imports_file "$file"
done

echo "All packages.py files have been processed."

# chmod +x ./devops/scripts/rewrite_imports_py.sh
# bash ./devops/scripts/rewrite_imports_py.sh