try: import packages # Import neccessary packages
except: pass
...
class LLMClient:
    def __init__(
        ...
        if os.getenv("PORT_SVC_LLM_NANO"):
            base_url = f'http://localhost:{int(os.getenv("PORT_SVC_LLM_NANO"))}'
    ...
    async def chat(
        ...
        Args:
            ...
            mode: Chat mode (sync/async/stream/astream)
            
        Returns:
            For streaming modes: AsyncGenerator yielding tokens
            For sync modes: ChatResponse object
        """
        ...
        try:
            if mode in [ChatMode.STREAM, ChatMode.ASTREAM]:
                response = await self._http_client.stream(
                    "POST",
...
""" Examples
client = LLMClient()
result = await client.chat("Tell me a joke", mode=ChatMode.SYNC)
